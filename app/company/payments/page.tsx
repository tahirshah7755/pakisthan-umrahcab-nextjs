...
export default function CompanyPaymentsPage() {
  ...
  const handleExportExcel = () => {
    ...
    const headers = ["Payment ID", "Date", "Payment Method", "Transaction ID", "Deposited Amount", "Credit Amount", "Currency", "Status"];
  ...
  const handlePrint = (title: string = "Corporate Balance Deposits Statement") => {
    ...
    const rowsHtml = filteredPayments.map((p: any) => {
      ...
      <tr>
        ...
        <td style="padding: 10px; border-bottom: 1px solid #e2e8f0; font-weight: bold; text-align: right; color: #10b981;">${!isLoan ? fmt(p.amount, p.currency) : "--"}</td>
        <td style="padding: 10px; border-bottom: 1px solid #e2e8f0; font-weight: bold; text-align: right; color: #ef4444;">${isLoan ? fmtNegative(p.amount, p.currency) : "--"}</td>
        ...
        <body>
          <div class="header">
            ...
            <div style="text-align: right;">
              ...
              <p><strong>Total Transactions:</strong> ${filteredPayments.length}</p>
            </div>
          </div>
          <table>
            <thead>
  ...
  const handleButtonClick = (exportFmt: string) => {
    if (exportFmt === "Copy") handleCopy();
    else if (exportFmt === "CSV") handleExportCSV();
    else if (exportFmt === "Excel") handleExportExcel();
    else if (exportFmt === "PDF" || exportFmt === "Print") handlePrint(exportFmt === "PDF" ? "Balance Deposits Statement - PDF Report" : "Balance Deposits Statement");
  };
  ...
  const fmt = (n: number, curr = "SAR") => `${curr} ${Number(n || 0).toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px", flexWrap: "wrap", gap: "15px" }}>
          <div className="mobile-toolbar" style={{ display: "flex", gap: "16px", alignItems: "center", flexWrap: "wrap" }}>
            <div style={{ display: "flex", gap: "6px" }}>
              {["Copy", "CSV", "Excel", "PDF", "Print"].map((exportFmt) => (
                <button
                  key={exportFmt}
                  onClick={() => handleButtonClick(exportFmt)}
              ...
              <table className="db-table" style={{ width: "100%" }}>
                ...
                <tbody>
                  {filteredPayments.length === 0 ? (
                    <tr>
                      <td colSpan={8} style={{ textAlign: "center", color: "#64748b", padding: "30px 10px" }}>No payments registered under this corporate account.</td>
                    </tr>
                  ) : (
                ...
                <tfoot>
                  <tr style={{ background: "#f8fafc", fontWeight: "bold", borderTop: "2px solid #e2e8f0" }}>
                    <td colSpan={4} style={{ padding: "12px 16px", textAlign: "right" }}>Filtered Totals:</td>
                    <td style={{ padding: "12px 10px", color: "#10b981" }}>
                      {fmt(calculateDepositedTotal(filteredPayments))}
                    </td>
                    <td style={{ padding: "12px 10px", color: "#ef4444" }}>
                      {fmtNegative(calculateCreditTotal(filteredPayments))}
                    </td>
                    <td colSpan={2}></td>
                  </tr>
                </tfoot>
