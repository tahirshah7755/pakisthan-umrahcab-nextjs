...
const fmtNegative = (n: number, curr = "SAR") =>
  `- ${curr} ${Math.abs(Number(n || 0)).toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;

const isLoanTransaction = (p: any) => {
  if (!p) return false;
  const methodLower = (p.method || "").toLowerCase();
  const proofLower = (p.proof_details || "").toLowerCase();
  const transRefLower = (p.transaction_ref || "").toLowerCase();
  return methodLower.includes("loan") || methodLower.includes("credit") || proofLower.includes("loan") || transRefLower.includes("loan");
};

const calculateDepositedTotal = (payments: any[]) => {
...
const calculateCreditTotal = (payments: any[]) => {
  if (!Array.isArray(payments)) return 0;
  return payments.reduce((acc: number, p: any) => {
    const statusLower = (p.status || "").toLowerCase();
    if (statusLower === "failed" || statusLower === "rejected" || statusLower === "cancelled") return acc;
...
const calculateTotalAmount = (payments: any[]) => calculateDepositedTotal(payments);
...
export default function PaymentsPage() {
  ...
  const handleCopy = () => {
    ...
    const headers = ["Ref ID", "Company", "Date", "Payment Method", "Transaction Ref", "Deposited Amount", "Credit Amount", "Currency", "Audit Status"];
    const textRows = paginatedPayments.map((p: any) => {
      const isLoan = isLoanTransaction(p);
      return [
  ...
  const handlePrint = (title: string = "Corporate Payments Audit Ledger") => {
    ...
    const rowsHtml = paginatedPayments.map((p: any) => {
      ...
      <tr>
        ...
        <td style="padding: 10px; border-bottom: 1px solid #e2e8f0; font-weight: bold; text-align: right; color: #059669;">${!isLoan ? fmt(p.amount, p.currency) : "--"}</td>
        <td style="padding: 10px; border-bottom: 1px solid #e2e8f0; font-weight: bold; text-align: right; color: #dc2626;">${isLoan ? fmtNegative(p.amount, p.currency) : "--"}</td>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "24px", padding: "10px" }}>
              ...
              <table className="db-table" style={{ margin: 0, fontSize: "12px" }}>
                      ...
                      return (
                        <tr key={p.id}>
                          ...
                          <td style={{ fontWeight: 800, color: "#059669" }}>
                            {!isLoan ? fmt(p.amount, p.currency) : <span style={{ color: "#94a3b8", fontWeight: 400 }}>--</span>}
                          </td>
                          <td style={{ fontWeight: 800, color: "#dc2626" }}>
                            {isLoan ? fmtNegative(p.amount, p.currency) : <span style={{ color: "#94a3b8", fontWeight: 400 }}>--</span>}
                          </td>
                          <td>
                            {p.currency}
                          </td>
                            ...
                            <select
                              ...
                              onChange={async (e) => {
                                ...
                                if (newStatus === "Approved") {
                                  ...
                                  setReceivedAmount(p.amount ? p.amount.toString() : "");
                                  setApprovalNotes("");
                ...
                <tfoot>
                  <tr style={{ background: "#f8fafc", fontWeight: "bold", borderTop: "2px solid #e2e8f0" }}>
                    <td colSpan={4} style={{ padding: "12px 16px", textAlign: "right" }}>Page Totals:</td>
                    <td style={{ padding: "12px 10px", color: "#059669" }}>
                      {fmt(calculateDepositedTotal(paginatedPayments))}
                    </td>
                    <td style={{ padding: "12px 10px", color: "#dc2626" }}>
                      {fmtNegative(calculateCreditTotal(paginatedPayments))}
                    </td>
                    <td colSpan={3}></td>
                  </tr>
                  <tr style={{ background: "#f1f5f9", fontWeight: "bold", borderTop: "1px solid #cbd5e1" }}>
                    <td colSpan={4} style={{ padding: "12px 16px", textAlign: "right" }}>Filtered Totals (All Pages):</td>
                    <td style={{ padding: "12px 10px", color: "#0d9488" }}>
                      {fmt(calculateDepositedTotal(allPayments))}
                    </td>
                    <td style={{ padding: "12px 10px", color: "#dc2626" }}>
                      {fmtNegative(calculateCreditTotal(allPayments))}
                    </td>
                    <td colSpan={3}></td>
                  </tr>
                </tfoot>
      ...
      {/* Premium Dynamic Approval Modal */}
      {showApprovalModal && selectedPayment && (
        <div style={{
          ...
          <div style={{
            ...
            {(() => {
              const mLower = (selectedPayment.method || "").toLowerCase();
              const isDue = mLower === "loan due" || mLower.includes("due");
              const isLoanReq = !isDue && (mLower.includes("loan") || mLower.includes("credit"));

              const headerBg = isDue 
                ? "linear-gradient(135deg, #059669, #047857)" 
                : isLoanReq 
                  ? "linear-gradient(135deg, #4f46e5, #3730a3)" 
                  : "linear-gradient(135deg, #0f766e, #0d9488)";

              const modalTitle = isDue 
                ? "Confirm Loan Repayment Received" 
                : isLoanReq 
                  ? "Approve Agent Loan Request" 
                  : "Approve Payment Deposit";

              const modalSub = isDue 
                ? "Record repayment received from agent to clear outstanding loan debt" 
                : isLoanReq 
                  ? "Grants booking credit to agent & creates tracking Loan Due record" 
                  : "Verify deposit proof & credit corporate booking balance";
                ...
                <>
                  ...
                  <div style={{
                    ...
                    <h3 style={{ margin: 0, fontSize: "18px", fontWeight: "700", display: "flex", alignItems: "center", gap: "10px" }}>
                      <i className={isDue ? "fas fa-hand-holding-dollar" : isLoanReq ? "fas fa-coins" : "fas fa-check-circle"} style={{ fontSize: "20px" }}></i>
                      {modalTitle}
                    </h3>
                  ...
                  <div style={{ padding: "24px", display: "flex", flexDirection: "column", gap: "20px" }}>
                    ...
                    <div style={{
                      ...
                      <div>
                        <span style={{ color: "#64748b", display: "block", fontSize: "11px", textTransform: "uppercase", fontWeight: "600" }}>{isDue ? "Outstanding Due Amount" : "Requested Amount"}</span>
                        <strong style={{ color: isDue ? "#dc2626" : "#0f766e", fontSize: "14px" }}>{fmt(selectedPayment.amount, selectedPayment.currency)}</strong>
                      </div>
                      <div>
                    ...
                    <div style={{
                      background: isDue ? "#f0fdf4" : isLoanReq ? "#eeef2e" : "#eff6ff",
                      backgroundColor: isDue ? "#f0fdf4" : isLoanReq ? "#f5f3ff" : "#eff6ff",
                      border: `1px solid ${isDue ? "#bbf7d0" : isLoanReq ? "#ddd6fe" : "#bfdbfe"}`,
                      borderRadius: "10px",
                      padding: "14px",
                      fontSize: "12px",
                      color: isDue ? "#166534" : isLoanReq ? "#4338ca" : "#1e40af"
                    }}>
                      <div style={{ fontWeight: "700", marginBottom: "6px", display: "flex", alignItems: "center", gap: "6px" }}>
                        <i className={isDue ? "fas fa-info-circle" : isLoanReq ? "fas fa-exclamation-circle" : "fas fa-info-circle"}></i>
                        <span>{isDue ? "Loan Repayment Impact:" : isLoanReq ? "Loan Request Approval Impact:" : "Deposit Approval Impact:"}</span>
                      </div>
                      {isDue ? (
                        <ul style={{ margin: 0, paddingLeft: "18px", display: "flex", flexDirection: "column", gap: "4px" }}>
                          <li>Confirms Admin received <strong>{fmt(Number(receivedAmount) || 0, selectedPayment.currency)}</strong> repayment from <strong>{selectedPayment.company}</strong>.</li>
                          <li>Clears agent's outstanding loan debt. <strong>No additional credit is added to agent balance</strong>.</li>
                          {Number(receivedAmount) < selectedPayment.amount ? (
                            <li>Remaining balance of <strong>{fmt(selectedPayment.amount - (Number(receivedAmount) || 0), selectedPayment.currency)}</strong> will stay as a new pending due entry.</li>
                          ) : (
                            <li>Full repayment locks this entry. <strong>No extra due fields will be created</strong>.</li>
                          )}
                        </ul>
                      ) : isLoanReq ? (
                        <ul style={{ margin: 0, paddingLeft: "18px", display: "flex", flexDirection: "column", gap: "4px" }}>
                          <li>Credits <strong>{fmt(Number(receivedAmount) || 0, selectedPayment.currency)}</strong> directly to <strong>{selectedPayment.company}</strong>'s booking wallet.</li>
                          <li>Generates an auto-tracking <strong>Loan Due</strong> entry (status Pending) so you can track agent repayment later.</li>
                        </ul>
                      ) : (
                        <ul style={{ margin: 0, paddingLeft: "18px", display: "flex", flexDirection: "column", gap: "4px" }}>
                          <li>Credits <strong>{fmt(Number(receivedAmount) || 0, selectedPayment.currency)}</strong> directly to <strong>{selectedPayment.company}</strong>'s booking wallet.</li>
                    ...
                    <div>
                      <label style={{ display: "block", fontSize: "13px", fontWeight: "700", color: "#334155", marginBottom: "6px" }}>
                        {isDue ? "Amount Received from Agent *" : "Approved Amount *"}
                      </label>
                      <div style={{ position: "relative" }}>
                        <span style={{
                          position: "absolute",
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        placeholder={isDue ? "e.g. Received cash payment at office counter." : "e.g. Verified bank transfer transaction ID."}
                        rows={2}
                        style={{
                          width: "100%",
                          padding: "10px 12px",
                  ...
                  <div style={{
                    ...
                    >
                      {isSubmittingApproval ? "Processing..." : isDue ? "Confirm Repayment Received" : "Confirm Approval"}
                    </button>
                  </div>
                </>
              );
